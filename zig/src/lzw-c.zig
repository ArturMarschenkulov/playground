...
pub const struct_singly_linked_list = extern struct {
    next: [*c]struct_singly_linked_list,
    data: c_int,
};
pub export fn insert_append(arg_anchor: [*c]struct_singly_linked_list, arg_newlink: [*c]struct_singly_linked_list) void {
    var anchor = arg_anchor;
    var newlink = arg_newlink;
    newlink.*.next = anchor.*.next;
    anchor.*.next = newlink;
}
pub const struct_elem = extern struct {
    data: c_int,
    next: [*c]struct_elem,
};
pub const cell = struct_elem;
pub const list = [*c]cell;
...
pub extern fn scanf(noalias format: [*c]const u8, ...) c_int;
pub extern fn atoi(str: [*c]const u8) c_int;
pub export fn addToList(arg_a: list, arg_num: c_int) list {
    var a = arg_a;
    var num = arg_num;
    var iter: list = undefined;
    var temp: list = undefined;
    var i: c_int = 0;
    _ = @TypeOf(i);
    if (a == null) {
        a = @as(list, @ptrCast(@alignCast(malloc(@sizeOf(cell)))));
        a.*.data = num;
        a.*.next = null;
    } else {
        iter = a;
        ...
        temp = @as(list, @ptrCast(@alignCast(malloc(@sizeOf(cell)))));
        temp.*.data = num;
        temp.*.next = null;
        iter.*.next = temp;
    }
...
pub export fn deleteFromList(arg_a: list, arg_pos: c_int) list {
    var a = arg_a;
    var pos = arg_pos;
    var i: c_int = 1;
    var temp: list = undefined;
    ...
    if (a != null) {
        ...
        if (pos == @as(c_int, 1)) {
            a = a.*.next;
            iter.*.next = null;
            free(@as(?*anyopaque, @ptrCast(iter)));
        } else {
            while ((blk: {
                const ref = &i;
                const tmp = ref.*;
...
pub export fn printList(arg_a: list) void {
    var a = arg_a;
    var temp: list = a;
    _ = printf("List contains following elements : \n");
    while (temp != null) {
        _ = printf("%d ", temp.*.data);
        temp = temp.*.next;
    }
}
pub export fn main(arg_argC: c_int, arg_argV: [*c][*c]u8) c_int {
    ...
    if (argC == @as(c_int, 1)) {
        _ = printf("Usage : %s <list of integers to be inserted into the list>", argV[@as(c_uint, @intCast(@as(c_int, 0)))]);
    } else {
        {
            i = 2;
            while (i <= argC) : (i += 1) {
                a = addToList(a, atoi((blk: {
        ...
        printList(a);
        while (true) {
            _ = printf("\nEnter position of element to be removed (1-%d) : ", argC - @as(c_int, 1));
            _ = scanf("%d", &i);
            if ((i > @as(c_int, 0)) and (i <= (argC - @as(c_int, 1)))) {
                a = deleteFromList(a, i);
                printList(a);
            }
            if (!((i > (argC - @as(c_int, 1))) or (i <= @as(c_int, 0)))) break;
        }
    }
    return 0;
}
...
pub const singly_linked_list = struct_singly_linked_list;
pub const elem = struct_elem;
